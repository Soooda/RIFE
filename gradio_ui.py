...
def infer(frame1, frame2, exp, ratio, rthreshold, rmaxcycles):
    img0 = (TF.pil_to_tensor(frame1) / 255.).to(device)
    img1 = (TF.pil_to_tensor(frame2) / 255.).to(device)
    ...
    if ratio:
        ...
        img_list.append(middle.detach()[0] * 255)
        img_list.append(img1.detach()[0] * 255)
    else:
        img_list = [img0, img1]
        for i in range(exp):
            tmp = []
            ...
            tmp.append(img1)
            img_list = tmp
    img_list = [img.detach()[0] * 255 for img in img_list]
...
with gr.Blocks() as demo:
    ...
    ''')
    with gr.Column():
        checkpoint = gr.Textbox(value="train_log", label="Checkpoint")
